...
//	private static final String PRINTCAP = "/etc/printcap";
	...
	private static final Logger log = Logger.getLogger(PrinterStatusThread.class);
    	...
    	try {
    		if (!machine.isEmpty() && !remotePrinter.isEmpty()) {
    			...
    			try {
    				...
    				log.error("IOException caught executing smbclient: "+ioe);
    				return false;
    			} catch (InterruptedException e) {
    				log.error("InterruptedException caught executing smbclient: "+e);
    				return false;
    			} catch (Exception e) {
	...
	public void run() {
		...
		try {
			...
			log.warn("Interruption occurred: "+ie);
		} catch (Exception e) {
