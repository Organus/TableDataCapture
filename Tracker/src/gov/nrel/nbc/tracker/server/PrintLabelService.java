...
import gov.nrel.nbc.tracker.dao.PrintersDAOHibernate;
...
public class PrintLabelService implements AppConstants {
	...
	private static final Logger log = Logger.getLogger(PrintLabelService.class);
    ...
    public String createDuplicateForm(int len, String group, LabelDTO label) {
	    ...
	    if (!DevTestProdConstants.DISPLAY_MODE.equals(DevTestProdConstants.ALGAE)) {
	        ...
	        "A445,208,0,1,1,1,N,\"Work ID: \"V13\n" +
	        "A415,233,0,2,1,1,N,\"DUPLICATE LABEL\"\n" +
        ...
        form += "A435,188,0,1,1,1,N,\"Custodian: \"V07\n" +
	        ...
	        "A390,238,0,1,1,1,N,\"Work ID: \"V14\n" +
	        "A385,258,0,2,1,1,N,\"DUPLICATE LABEL\"\n" +
	        "FE\n" + NEW_LINE;    	
	    }
	    return form;
    }
    ...
    public String createLabel(LabelDTO label, String group) {
    	...
    	//if (!DevTestProdConstants.DISPLAY_MODE.equals(DevTestProdConstants.ALGAE)) {
		    try {
		    	...
		    	log.warn("Problem parsing TRB number.");
		    }
		    
		    try {
		    	trbPageInt = Integer.parseInt(label.getTrbPage());
		    } catch (NumberFormatException nfe) {
		    	log.warn("Problem parsing TRB page.");
		    }
    	//}
	    ...
	    if (actual != null && !actual.isEmpty()) {
	    	...
	    	if (index < 1 || actual.length() < index+1) {
		    	...
		    	descriptor2 = actual.substring(index);
		    	if (descriptor2.length()>MAX_PER_LINE) {
		    		int index1 = getSplitIndex(descriptor2);
			    	if (index1 > 1) {
			    		//log.warn("index1="+index1+";desc2="+descriptor2);
				    	descriptor3 = descriptor2.substring(index1);
	    ...
	    if (specific != null) {
	    	if (specific.equals("O")) 
	    		specific = "OX";
	    	else if (specific.equals("A")) 
	    		specific = "ACID";
	    	else if (specific.equals("C"))
	    		specific = "CORR";
	    }
	    String workId = label.getWorkId();
	    if (workId == null)
	    	workId = "";
	    String s =      
	    	    //"N\n" +         // Clear Image Buffer  
    ...
    public boolean printFile(String filename, String printer) throws Exception {
    	...
    	//while (ret == -1) {
    		try {
    			...
    			//log.warn("Exception caught deleting file: "+e1);
    		}
    	//}
    	return worked;
    }
    ...
    private String getInfo(String printer, int which) {
		...
		try {
			...
			log.warn("problem reading "+PRINTCAP+" file: "+ioe);
		} finally {
			try {
		        if (bis != null)
		        	bis.close(); 
			} catch (IOException ioe1) {
	    		log.warn("problem closing "+PRINTCAP+" file: "+ioe1);
			}
		}
	...
	private boolean testPrinter(String printer) {
		...
		try {
    		...
    		log.error(TrackerServiceImpl.getStackTrace(he));
        	try {
    		...
    		log.error(TrackerServiceImpl.getStackTrace(ex));
    		return ok;
        } finally {
        	if (session != null && session.isConnected())
        		if (session.isOpen())
    ...
    public ArrayList<String> listPrinters() {
	    ...
	    try {
			...
			log.error("IOException caught reading "+PRINTCAP+" file: "+e);
	    } catch (Exception e) {
	        log.warn(TrackerServiceImpl.getStackTrace(e));
	    } finally {
	    	try {
	    		if (bis != null)
	    			bis.close();
	    	} catch (IOException ioe) {
	    		log.warn("problems closing "+PRINTCAP+": "+ioe);
	    	}
	    }
	    return printers;
    }
	    ...
	    try {
	    	...
	    	if (s != null) {
		    	...
		    	if (os != null && os.toLowerCase().indexOf(WINDOWS) != -1) {
	    			...
	    			System.out.println("trying to print to "+sPrinterName);
	    			log.warn("trying to print to "+sPrinterName);
