...
public class ExcelSXParserTest extends TestCase implements AppConstants {
	ClassLoader thisLoader;
	ExcelSXParser parser;
	String configName;
	String sheetName;
	String fileName;
	Session session;
	...
	protected void setUp() {
    	thisLoader = ExcelSXParser.class.getClassLoader();
		...
		fileName = System.getProperty("java.io.tmpdir") + "nir2-b.xlsx";
...
*/	
	...
	public static Test suite() {
		return new TestSuite(ExcelSXParserTest.class);
	}

	public void testGetSheets() throws Exception {
		
		parser = new ExcelSXParser(f,null,session);
	...
	public void testGetMetadataNames() throws Exception{
		...
		parser = new ExcelSXParser(f,config,session);
	...
	public void testGetData() throws Exception{
		...
		assertTrue(rows.get(rowCount-1).getRow().get(1).getValue().getSvalue().equals(""));
...
//		System.out.println(rows.get(rowCount-1).getRow().get(headerCount -1).getValue().getRvalue());
		assertTrue(rows.get(rowCount-1).getRow().get(headerCount -1).getValue().getRvalue()==0.0);
