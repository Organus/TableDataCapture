...
public class DeleteConfiguration extends Composite implements ClickHandler,
		ChangeHandler, AppConstants {

	private static final String CONFIG_ID_COLON = "Configuration Name:";
	 ...
	 */
	private final String SUB_TITLE = "Delete Configuration";
	...
	private final FilteredTextBox wbConfigTextBox = new FilteredTextBox();
	 ...
	 */
	private void submitDeleteConfig() {
		final String wbConfig = wbConfigTextBox.getText();
			asa.deleteWorkbookConfig(wbConfig, new AsyncCallback<Boolean>() {

				@Override
				public void onFailure(Throwable caught) {
					Window.alert("There was a problem deleting the configuration. Error: " + caught.getLocalizedMessage());
				}

				@Override
				public void onSuccess(Boolean result) {
					wbConfigTextBox.setValue("");
					submitButton.setEnabled(false);
					Window.alert("The configuration, " + wbConfig + " was deleted.");
				}
			});
	}
	...
	private void enableSubmitButton() {
		String value = wbConfigTextBox.getText();
		submitButton.setEnabled(!value.isEmpty());
	}
}
