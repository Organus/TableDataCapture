 ...
 *
 */
public class SearchSample extends Composite implements AppConstants, ClickHandler {
	...
	private static final String SAMPLE_GRID_HEIGHT = "200px";
	...
	private static final int GRID_COLUMNS = 31;
	...
	private static String[] headerList = {PAGE_LABEL,	 NUMBER_LABEL,	 SHELF_LABEL,	 HOLDER_LABEL,	 ROOM_LABEL,	 BUILDING_LABEL,	 STATUS_LABEL,	 FRACTION_LABEL,	 	 FORM_LABEL,	 DESTINATION_LABEL_ALGAE,	 DESTINATION_LABEL_SABC,	 STRAIN_LABEL,	 FEEDSTOCK_LABEL,	 TREATMENT_LABEL,	 CUSTODIAN_NAME_LABEL,	 COMPOSITION_LABEL,	 EXTERNAL_ID_LABEL,	 DATE_CREATED_LABEL,	 AMOUNT_LABEL,	 UNITS_LABEL, COMMENTS_LABEL,	 DESCRIPTOR_LABEL,	 SAMPLE_ID_LABEL,	 TRACKING_ID_LABEL,	 OWNER_LABEL,	 SUBLOCATION_LABEL,	 PACKAGING_LABEL,	 STORAGE_NOTES_LABEL,	 ORIGIN_LABEL};
	private static final String[] algaeList = {	 SAMPLE_ID_LABEL,	 TRACKING_ID_LABEL,	 OWNER_LABEL,	 PAGE_LABEL,	 NUMBER_LABEL,	 ORIGIN_LABEL, STATUS_LABEL,	 FRACTION_LABEL, BIOMASS_LABEL,FORM_LABEL,DESTINATION_LABEL_ALGAE,	 STRAIN_LABEL,	 CUSTODIAN_NAME_LABEL,	 COMPOSITION_LABEL,	 DATE_CREATED_LABEL,	 AMOUNT_LABEL,	 UNITS_LABEL, COMMENTS_LABEL,	 DESCRIPTOR_LABEL};
	private static final String[] biomassList = {	 SAMPLE_ID_LABEL,	 TRACKING_ID_LABEL,	 OWNER_LABEL,	 PAGE_LABEL,	 NUMBER_LABEL,	 ORIGIN_LABEL ,SHELF_LABEL,	 HOLDER_LABEL,	 ROOM_LABEL,	 BUILDING_LABEL, SUBLOCATION_LABEL,	 PACKAGING_LABEL,	 STORAGE_NOTES_LABEL,STATUS_LABEL,	 FRACTION_LABEL, DESTINATION_LABEL_ALGAE,	 FEEDSTOCK_LABEL,	 TREATMENT_LABEL,	 CUSTODIAN_NAME_LABEL,	 EXTERNAL_ID_LABEL,	 DATE_CREATED_LABEL,	 AMOUNT_LABEL,	 UNITS_LABEL, COMMENTS_LABEL,	 DESCRIPTOR_LABEL};
	...
	private static final String[] stringHeaders = {SHELF_LABEL,	 HOLDER_LABEL,	 ROOM_LABEL,	 BUILDING_LABEL,	 FRACTION_LABEL,	 BIOMASS_LABEL,	 FORM_LABEL,	 DESTINATION_LABEL_ALGAE,	 DESTINATION_LABEL_SABC,	 STRAIN_LABEL,	 FEEDSTOCK_LABEL,	 TREATMENT_LABEL,	 CUSTODIAN_NAME_LABEL,	 "Composition",	 EXTERNAL_ID_LABEL,COMMENTS_LABEL,	 DESCRIPTOR_LABEL,	 SAMPLE_ID_LABEL,  OWNER_LABEL,	 SUBLOCATION_LABEL,	 PACKAGING_LABEL,	 STORAGE_NOTES_LABEL,	 ORIGIN_LABEL, UNITS_LABEL};
	...
	private void loadValues(String headerName, MultiWordSuggestOracle suggestOracle) {
		...
		else if (headerName.equals(SearchSample.DESTINATION_LABEL_ALGAE)||headerName.equals(SearchSample.DESTINATION_LABEL_SABC))
		...
		else if (headerName.equals(SearchSample.UNITS))
		...
		else if (headerName.equals(SearchSample.HOLDER))
	...
	*/
	...
	private SampleCriteria setValue(SampleCriteria crit,String header,String value) {
		if (header.equals(SearchSample.SAMPLE_ID_LABEL))
		...
		else if (header.equals(SearchSample.DESTINATION_LABEL_ALGAE)||header.equals(SearchSample.DESTINATION_LABEL_SABC))
		...
		else if (header.equals(SearchSample.UNITS))
			crit.setUnits(value);
			...
			crit.setShelf(value);
		else if (header.equals(SearchSample.HOLDER))
	...
	private class SampleSearchCallback implements AsyncCallback<Collection<SampleCriteria>> {
		...
		private static final String HOLDER_EADING = "Holder";
		...
		private static final String DESTINATION_HEADING = "Project\nTask No.";
		...
		private void loadResultsTable(final Collection<SampleCriteria> result, String group, String sessionId) {
			...
			col++;
			if (DevTestProdConstants.DISPLAY_MODE.equals(DevTestProdConstants.ALGAE)) {
				resultsGrid.setText(row, col, DESTINATION_HEADING);
				resultsGrid.getCellFormatter().setHorizontalAlignment(row, col, HasHorizontalAlignment.ALIGN_CENTER);
				col++;				
			}
			...
			if (!DevTestProdConstants.DISPLAY_MODE.equals(DevTestProdConstants.ALGAE)) {
				...
				resultsGrid.getCellFormatter().setHorizontalAlignment(row, col, HasHorizontalAlignment.ALIGN_CENTER);
				col++;
				resultsGrid.setText(row, col, HOLDER_EADING);
				resultsGrid.getCellFormatter().setHorizontalAlignment(row, col, HasHorizontalAlignment.ALIGN_CENTER);
				col++;
			...
			while (it.hasNext()) {
				...
				*/
				...
				resultsGrid.getCellFormatter().setWordWrap(row, col, false);
				resultsGrid.getCellFormatter().setHorizontalAlignment(row, col++, HasHorizontalAlignment.ALIGN_CENTER);
				resultsGrid.setText(row, col, sample.getDescription() != null && sample.getDescription().length() > 0 ? sample.getDescription() : BLANKS);
				resultsGrid.getCellFormatter().setWordWrap(row, col, false);
				resultsGrid.getCellFormatter().setHorizontalAlignment(row, col++, HasHorizontalAlignment.ALIGN_CENTER);
				row++;
			}
	...
	private void handlePageButtonClick() {
		...
		List<SampleCriteria> crits = new ArrayList<SampleCriteria>(); 
		...
		tsa.searchSamples(crits, sessionId, position, PAGE_SIZE, new SampleSearchCallback());
		...
		for (int row = 1; row < criteriaGrid.getRowCount(); row++) {
			...
			if (idx > 0) {
				...
				if (widgetName.endsWith(DATE_BOX)) {
					DateBox dateBox = (DateBox) criteriaGrid.getWidget(row, 2);
					//trio.setValue(DateTimeFormat.getShortDateFormat().format(dateBox.getValue()));
					crit=setValue(crit,header,fmt.format(dateBox.getValue()));
				} else if (widgetName.endsWith(TEXT_BOX)) {
					FilteredTextBox textBox = (FilteredTextBox) criteriaGrid.getWidget(row, 2);
					crit = setValue(crit,header,textBox.getValue());
					//trio.setValue(textBox.getValue());
