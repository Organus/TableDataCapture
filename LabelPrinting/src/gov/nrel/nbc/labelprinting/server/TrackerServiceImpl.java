...
import gov.nrel.nbc.labelprinting.client.TrackerService;
...
import gov.nrel.nbc.labelprinting.dao.AuditDAOHibernate;
import gov.nrel.nbc.labelprinting.dao.CompositionsDAOHibernate;
import gov.nrel.nbc.labelprinting.dao.CustodiansDAOHibernate;
import gov.nrel.nbc.labelprinting.dao.DestinationsDAOHibernate;
import gov.nrel.nbc.labelprinting.dao.FeedstocksDAOHibernate;
import gov.nrel.nbc.labelprinting.dao.FormsDAOHibernate;
...
import gov.nrel.nbc.labelprinting.dao.StrainsDAOHibernate;
...
public class TrackerServiceImpl extends RemoteServiceServlet implements
	...
	public Collection<String> getSampleIds(String id) {
		...
		try {
			...
			SecurityServiceImpl ssi = new SecurityServiceImpl();
			List<Long> sids = ssi.getObjectIdsForUser(id, "sample");		
			sampleNames = sdh.findAllSampleIds(sids);
	...
	private Sample setCriteria(Sample theSample, SampleCriteria criteria) {
		...
		if (criteria.getDescription() != null) theSample.setDescription(criteria.getDescription().indexOf(SINGLE_QUOTE) == -1 ? criteria.getDescription() : criteria.getDescription().replaceAll(SINGLE_QUOTE, FIXED_QUOTE));
		else theSample.setDescription(null);
	...
	private Sample setLabelCriteria(Sample theSample, SampleCriteria criteria) {
		...
		if (criteria.getDescription() != null) 
			theSample.setDescription(criteria.getDescription().indexOf(SINGLE_QUOTE) == -1 ? criteria.getDescription() : criteria.getDescription().replaceAll(SINGLE_QUOTE, FIXED_QUOTE));
		else
			theSample.setDescription("");
    ...
    public String createLabelImage(Sample sample) {
		...
		try {
		    ...
		    String descriptor3 = "";
		    if (sample != null) {
			    String actual = sample.getDescription();
	...
	public Boolean emailLabel(String sessionId, LabelDTO label) {
		...
		try {
			...
			if (sample.getDescription() != null) label.setDescription(sample.getDescription());
	...
	public Boolean printLabel(String sessionId, LabelDTO label, String printerName) {
		...
		try {
			...
			boolean ret = pls.printLabel(label, printerName, "SABC");
	...
	private boolean insertLabelToPrint(gov.nrel.nbc.labelprinting.model.Sample sample, String sessionId, String printer) {
