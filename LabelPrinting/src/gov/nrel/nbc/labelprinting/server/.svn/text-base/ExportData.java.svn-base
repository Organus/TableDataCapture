...
public class ExportData implements AppConstants {
     ...
     */
	private final String[] headers = {"Tracking ID","Sample ID","External ID","Owner Name","Custodian Name","Status","TRB Number","TRB Page","Treatment","Feedstock","Fraction","Fire","Reactivity","Specific","Health","Approximate Amount","Units","Building","Room","Sublocation","Shelf","Holder","Packaging","Storage Notes","Date Entered","Comments"};
	private final String[] types = {LONG,           STRING,     STRING,       STRING,      STRING,           STRING,  LONG,        LONG,      STRING,     STRING,     STRING,   LONG,    LONG,       STRING,      LONG,      LONG,             STRING,   STRING,   STRING,  STRING,      STRING, STRING,   STRING,     STRING,         DATE,         STRING};
	...
	{		
		...
		while (it.hasNext()) {
			...
			String[] values = {String.valueOf(trackingId),sampleId,extId,own,cust,status,String.valueOf(num),String.valueOf(page),treatment,feed,fraction,fire,reactivity,specific,health,amount,units,
					building,room,subloc,shelf,holder,pkging,snotes,sdate,comments};
